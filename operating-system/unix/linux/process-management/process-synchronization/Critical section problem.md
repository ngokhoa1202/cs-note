...
## Rationale
- The statement `count++`  and `count--` are ultimately translated into machine language. It is the Arithmetic Logic Unit that loads the value of `count` variable into a specific register and performs a binary operation on it.
```c
// count++
register1 = count
...
```C title='Critical section problem'
while (true) {
  /* entry section */
