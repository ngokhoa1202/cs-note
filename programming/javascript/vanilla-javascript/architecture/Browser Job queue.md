...
- For example, tasks is added to the task queue when:
	...
	- An event occurs; for instance, the user clicks an element on the webpage. A task is then created and executes all event callbacks.
...
- If a task takes too long, then the browser will <mark class="hltr-yellow">be blocked from executing other tasks</mark>. So after some time, it raises an alert like “Page Unresponsive”, suggesting killing the task with the whole page. That occurs when there are much *complex computation* or a programming error leading to an *infinite loop*.
# Micro-task queue
- Micro-tasks are callbacks executed as `Promise` handlers such as `.then`, `catch` and `finally`.
- When a promise is ready, its handlers are enqueued into micro-task queue (also known as `PromiseJobs` queue). It is, nevertheless, not until the JavaScript call stack is empty that these handlers are taken from the queue and executed.
- At the *micro-task checkpoint* (which happens after the current task finishes, before the browser continues to the next task, such as a timer or I/O event, and before rendering), the browser drains the entire micro-task queue.
