...
# Use --watch flag
- Use flag `--watch` to put a `.ts` file under watch mode compilation. 
```bash
...
```
- Example
```json title='Simple ts.config.json'
{
	...
	"watchOptions": {
		// Use native file system events for files and directorie		
		...
		// Finally, two additional settings for reducing the amount of possibl
