...
# Library
- If the `lib` property is not set, the compiler implicitly uses the library of the Javascript environment defined in the `target` property.
# Javascript file allowance
- Property `allowJs` and `checkJs`
- If these two properties are set, the compiler performs static checking on Javascript files.
# Source file mapping
- Property `sourceMap`
- By default, the Typescript source code is compiled into a Javascript file and not shown on the browser source code.
- Specify this property to map the typescript source into a source file the browser.
- Used for debugging Typescript files.
# Project organization
## Directory
- Property `rootDir` and `outDir`
- `outDir` specifies an out directory for emitted Javascript files.
- `rootDir` specifies the root directory for current project, which contains only Typescript files.
## Involved files
- Property `include`, `exclude` and `files`.
- Property `include` is used to specify directories or files that need compiling.
- Property `exclude` is used to specify directories or files that need ignoring.
- Property `files` is used to specify some specific files without normal directory.
```json
`{    
  "include": [
    "src/**/*", "tests/**/*"
...
- `downLevelIteration` specifies special compiler's behavior when dealing with complicated loops; however, when being turned on, more output code is generated.
## Stop emitting when errors occur
