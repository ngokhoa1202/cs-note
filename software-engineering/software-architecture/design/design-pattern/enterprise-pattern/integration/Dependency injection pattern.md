...
# Application
- There are *multiple concrete implementations* of a same interface. These implementations must be <mark class="hltr-yellow">flexibly swapped</mark> without modifying the consuming class.
- The system needs different <mark class="hltr-yellow">configuration</mark> across environment.
- <mark class="hltr-yellow">Cross-cutting concerns</mark> such as logging, security and transaction management must be centralized.
# Components
```mermaid
classDiagram
    %% Abstraction Layer
...
2. [[software-engineering/software-architecture/principles/SOLID#Dependency inversion principle|SOLID]] for Inversion of Control.
3. 