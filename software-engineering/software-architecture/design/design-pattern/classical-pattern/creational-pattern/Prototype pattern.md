...
# When to use
- The system avoids large parallel subclasses in class hierarchy and only uses `clone` method for simplicity.
...
```
- Two concrete Prototypes `Swordsman` and `General` overrides method `reset`.
```Java
package com.coffeepoweredcrew.prototype;
