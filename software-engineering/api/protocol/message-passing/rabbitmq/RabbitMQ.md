#messaging #rabbitmq #amqp #message-broker #distributed-systems
==RabbitMQ== is an open-source message broker that implements the Advanced Message Queuing Protocol (AMQP) and supports multiple messaging protocols. It provides robust message routing, delivery guarantees, and enterprise-grade reliability for distributed systems.
...
### Management Plugin
Web-based UI and REST API for monitoring and administration.
...
### Key Metrics
- **Queue depth**: Unacknowledged message count
- **Consumer utilization**: Active vs idle consumers
- **Publish/deliver rates**: Throughput monitoring
- **Connection count**: Client session tracking

## Protocol Support

- **AMQP 0-9-1**: Primary protocol
- **STOMP**: Text-oriented messaging
- **MQTT**: IoT and mobile messaging
- **HTTP/REST**: Management and basic messaging

## Use Cases

### Task Distribution
Work queues distribute tasks across multiple workers with competing consumers pattern.

### Event Broadcasting
Fanout exchanges broadcast domain events to multiple interested services.

### Routing and Filtering
Topic exchanges enable sophisticated routing based on message classification.

### RPC Pattern
Request-reply messaging with correlation IDs and reply-to queues.
...
Set max-length or TTL to prevent memory exhaustion.

### Missing Consumer Acknowledgments
Leads to memory leaks as unacknowledged messages accumulate.

### Synchronous Operations in High-Throughput Scenarios
Use async confirms instead of `waitForConfirms()` for better performance.

### Ignoring Monitoring
Queue depth and consumer utilization are critical indicators of system health.
