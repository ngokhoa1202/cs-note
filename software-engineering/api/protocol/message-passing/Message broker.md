#software-architecture #messaging #distributed-systems #asynchronous #integration-pattern

- A ==message broker== is an intermediary software component that enables applications, services, and systems to communicate and exchange information by translating messages between formal messaging protocols. It acts as a middleman that receives messages from publishers and routes them to appropriate subscribers.
## Core Concepts
### Message-Oriented Middleware
Message brokers implement ==message-oriented middleware (MOM)== architecture, providing asynchronous communication between distributed components. Unlike synchronous request-response patterns, message brokers decouple producers from consumers in both time and space.
...
Asynchronous RPC pattern where client sends request with callback queue information for receiving responses.
...
### Direct Routing
Messages route to queues based on exact routing key match.
...
### [[RabbitMQ]]
AMQP-based broker with sophisticated routing, support for multiple protocols, and strong consistency guarantees.

### [[Apache Kafka]]
Distributed event streaming platform optimized for high-throughput, log-based persistence, and event replay capabilities.

### [[Solace]]
Enterprise-grade messaging platform supporting multiple protocols, WAN optimization, and hybrid cloud deployments.

### Others
- **Apache ActiveMQ**: JMS-compliant broker for Java ecosystems
...
### Message Broker as Database
Avoid storing long-lived data in queues. Use databases for persistence and brokers for transient messaging.

### Tight Coupling Through Messages
Messages should carry data, not implementation details. Avoid exposing internal service structure through message schemas.

### Unbounded Message Growth
Implement TTL, size limits, and dead letter queues to prevent resource exhaustion.

### Ignoring Idempotency
Always design consumers to handle duplicate messages gracefully when using at-least-once delivery.
